...
fn music_app_routes_use_sdkwork_v3_prefixes_and_resource_operation_ids() {
    ...
    assert!(routes.iter().any(|route| route.method == HttpMethod::Get && route.operation_id == "ai.stylePresets.list"));
    assert!(routes.iter().any(|route| route.method == HttpMethod::Get && route.operation_id == "ai.promptTemplates.list"));
    assert!(routes.iter().any(|route| route.method == HttpMethod::Post && route.operation_id == "ai.generation.tasks.create"));
